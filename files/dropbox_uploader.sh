...
RESPONSE_FILE="$TMPDIR/du_resp_$RANDOM"
CHUNK_FILE="$TMPDIR/du_chunk_$RANDOM"
TEMP_FILE="$TMPDIR/du_tmp_$RANDOM"
...
#Check temp folder
if [[ ! -d "$TMPDIR" ]]; then
    echo -e "Error: the temporary folder $TMPDIR doesn't exists!"
    echo -e "Please edit this script and set the TMPDIR variable to a valid temporary folder to use."
    exit 1
fi
...
if [[ $DEBUG != 0 ]]; then
    ...
    set -x
    RESPONSE_FILE="$TMPDIR/du_resp_debug"
fi
...
function db_list_outfile
{
    ...
    OUT_FILE="$TMPDIR/du_tmp_out_$RANDOM"
